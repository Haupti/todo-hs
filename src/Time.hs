...
import Data.Time (getCurrentTime, getTimeZone, utcToLocalTime, toGregorian, localDay, localTimeOfDay)
import Classes (Presenter(..), PresentableProvider (providePresentable)) 
...
instance Presenter LocalTime where
    present (LocalTime date h m s) = putStrLn $ providePresentable date ++ " " ++ show h ++ ts ++ show m ++ ts ++ show s
